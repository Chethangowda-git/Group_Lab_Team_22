...
/**
 * Panel to create a new course offering Registrar can select course, faculty,
 * and set capacity
 */
public class CreateCourseOfferingPanel extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        add(lblMessage);
        lblMessage.setBounds(30, 320, 500, 25);

        add(btnCreate);
        btnCreate.setBounds(200, 370, 150, 35);
    ...
    private void loadFaculty() {
        ...
        FacultyDirectory fd = business.getFacultyDirectory();  // FIXED - no "Business.Profiles."
   ...
   private void btnCreateActionPerformed(java.awt.event.ActionEvent evt) {
    ...
    // Get course number from dropdown (format: "INFO5100 - Application Engineering")
    String selectedCourseDisplay = (String) cmbCourse.getSelectedItem();
    String courseNumber = selectedCourseDisplay.split(" - ")[0]; // Extract "INFO5100"
    
    // Get selected faculty (if any)
    ...
    if (hasFaculty) {
        // Find the faculty profile by name
        FacultyDirectory fd = business.getFacultyDirectory();
        ArrayList<FacultyProfile> facultyList = fd.getFacultyList();
        
        for (FacultyProfile fp : facultyList) {
            if (fp.getPerson().getFullName().equals(selectedFacultyDisplay)) {
                // Assign this faculty to the course
                // We need to use University Model's FacultyProfile
    ...
    // 5. SUCCESS!
    System.out.println("✅ Created course offering: " + courseNumber + " with capacity " + capacity);
