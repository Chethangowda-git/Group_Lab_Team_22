...
/**
 * Financial Reconciliation Panel Monitor tuition payment status and generate
 * financial reports
 */
public class FinancialReconciliationPanel extends javax.swing.JPanel {
    ...
    private void loadFinancialData() {
        ...
        for (StudentProfile sp : students) {
            ...
            // Get actual payment status from student profile
          // Get actual payment status from student profile
double studentBalance = sp.getTuitionBalance();

// Calculate amount paid
int amountPaid = (int)(studentTuition - studentBalance);
if (amountPaid < 0) amountPaid = 0;

int balance = (int)studentBalance;
String status = balance > 0 ? "Unpaid" : "Paid";

            System.out.println("   " + sp.getPerson().getFullName()
                    + " | Owed: $" + studentTuition
                    + " | Balance: $" + balance
                    + " | Paid: $" + amountPaid);
            // Add to table
